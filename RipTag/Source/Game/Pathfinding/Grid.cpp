	...
	if (!m_roomNodeMap.empty() && !_tilesAreInTheSameRoom(source, destination))
	{
		...
		for (int i = 0; i < 101; i++)
		{
			for (int j = 0; j < 101; j++)
			{
				...
				if (m_nodeMap.at(j + i * 101).tile == source)
				{
					found = true;
					map << "O";
				}
				else if (m_nodeMap.at(j + i * 101).tile == destination)
				{
					found = true;
					map << "D";
				}
				if (!found)
					...
					{
						if (m_nodeMap.at(j + i * 101).tile == p.destination)
						{
							found = true;
							map << tpCount++;
							break;
						}
						else if (m_nodeMap.at(j + i * 101).tile == p.source)
						{
							found = true;
							map << tpCount++;
							break;
				...
				if (!found)
				{
					if (m_nodeMap.at(j + i * 101).tile.getPathable())
						map << " ";
					else
						map << "#";
				}
			...
			for (int i = 0; i < 101; i++)
			{
				for (int j = 0; j < 101; j++)
				{
					...
					if (!found)
						...
						{
							if (m_nodeMap.at(j + i * 101).tile == p->tile)
							{
								found = true;
								map << "X";
								break;
